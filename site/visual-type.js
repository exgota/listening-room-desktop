// Type: placeholder while the real visualizer is built.
registerVisualizer({
  key: "type",
  name: "Type",
  order: 2,
  create() {
    ...
    return {
      ...
      render(time, frame) {
        ...
        context.fillStyle = "#1f2bf0";
        context.fillRect(0, 0, frame.width, frame.height);
        context.fillStyle = "#ffffff";
        context.font = "600 40px 'IBM Plex Mono'";
        context.fillText("Type " + time.toFixed(2) + (song ? " " + song.scene(time).kind : " (no song)"), 80, frame.height - 300);
        if (song) {
