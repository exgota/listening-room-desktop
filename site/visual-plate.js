...
uniform float uUnit;  // canvas pixels per CSS pixel
void main() {
  vec2 px = uUnit / uResolution;
  ...
  create() {
    ...
    function render(time, frame) {
      const width = frame.width,
        height = frame.height;
      size = { width, height };
      // The plate: a square as tall as the sheet allows (above the controls while paused), in
      // the middle beside the title; when the title reaches past its left edge, moved right to
      // clear it, and only put under the title if the frame is too narrow for that.
      const bottom = height * (frame.playing === false ? 0.84 : 0.9);
      let top = height * 0.05;
      let side = Math.min(bottom - top, width * 0.9);
      let x0 = (width - side) / 2;
      const titleRight = (frame.titleRight || 0) + 44;
      if (titleRight > x0 && (frame.titleBottom || 0) > top) {
        const room = width - width * 0.03 - titleRight;
        if (room >= side * 0.85) {
          side = Math.min(side, room);
          x0 = Math.max(titleRight, (width - side) / 2);
        } else {
      ...
      gl.uniform1f(ground.uniforms.uUnit, canvas.width / width);
      ...
      if (drop.phase === 1) {
        ...
        // the first downbeat takes a visible step; the rest follow bar by bar
        gather = 0.88 * clamp01((done + (done > 0 ? 1 : 0)) / (endBar - startBar + 1));
      }
      if (drop.phase === 2) {
      ...
      let heavy = 0;
      {
        ...
        // a strong note's weight: in over 60 ms, held 150 ms past its end, out over 0.2 s; a
        // note that starts while the one before still weighs takes over from that weight
        const weightOf = (index) => {
          const attack = easeOutCubic(clamp01((time - notes.start[index]) / 0.06));
          const hold = notes.end[index] + 0.15;
          return time <= hold ? attack : attack * (1 - easeInOutCubic(clamp01((time - hold) / 0.2)));
        };
        let index = notes.last(time);
        while (index >= 0 && notes.strength[index] <= 0.3) index--;
        if (index >= 0) {
          heavy = weightOf(index);
          let before = index - 1;
          while (before >= 0 && notes.strength[before] <= 0.3) before--;
          if (before >= 0 && notes.start[index] <= notes.end[before] + 0.35) heavy = Math.max(heavy, Math.min(1, weightOf(before)));
        }
      }
      ...
      if (time < 0.5) heavy = 1; // the thumbnail
      ...
      // except in a drop's hold and in the first frames (the thumbnail)
      const sandPresence = holding || time < 0.5 ? 1 : (1 - 0.55 * voicePresence) * (1 - 0.8 * keyMoment);
