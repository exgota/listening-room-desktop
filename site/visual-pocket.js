...
const pocketSongs = {
  ...
  f127a026dc751f1528bfb95d: { field: "mint", order: ["bass", "singer", "drum"], main: "#12593f" }, // Ophelia: bottle green
  ...
  create() {
    ...
    function findSegments(model) {
      ...
      for (const boundary of all) {
        ...
        let feature = null;
        if (scene.kind === "verse" || scene.kind === "break") feature = phrase % 2 ? "singer" : null;
        else if (scene.kind === "gap") feature = result.length ? result[result.length - 1].feature : null; // the gap holds
        else if (scene.kind !== "drop" && scene.kind !== "build") feature = [null, "drum", null, "bass", null, "singer"][phrase % 6];
    ...
    function layoutFor(index, left, right, room, unit, poster = false) {
      const cacheKey = poster ? `poster${index}` : index;
      ...
      if (poster) names.splice(Math.ceil(names.length / 2), 0, "spacer");
      ...
      const sizes = names.map((name) => (name === "spacer" ? 240 : Math.min(base(name) * fit, columnCap(segment, name, room), 1.6)));
      ...
      names.forEach((name, column) => {
        if (name !== "spacer") result[name] = { x: left + (x + widths[column] / 2) * unit, size: sizes[column] };
    ...
    function colorsFor(fieldColor) {
      ...
      return {
        ...
        singer: near(pocketInk.cobalt) || (dark && b > r && b > g) ? pocketInk.cream : dark ? "#6f8ef7" : pocketInk.cobalt,
    ...
    function render(time, frame) {
      ...
      const posterOut = time >= finale ? easeInOutCubic(clamp01((time - finale) / 0.4)) : 0;
      ...
      // pale fields step through a partner tone every eight bars, so a long scene moves
      if (pocketPale.includes(fieldName) && Math.floor(Math.max(0, song.barIndex(time)) / 8) % 2 === 1 && drop.phase !== 1)
        fieldColor = mixColor(fieldColor, hexColor(pocketFields[pocketPale[(pocketPale.indexOf(fieldName) + 3) % 5]]), 0.55);
      const baseColors = colorsFor(fieldColor);
      if (windUp > 0) {
        const step = 0.55 * barsDone;
        fieldColor = baseColors.dark ? mixColor(fieldColor, hexColor(pocketInk.tomato), 0.6 * step) : mixColor(fieldColor, hexColor(pocketFields.night), step);
      }
      ...
      const grow = drop.phase === 3 && drop.index === song.mainDrop ? 1 + 0.18 * (1 - easeInOutCubic(sinceDrop / song.barPeriod)) : 1;
      ...
      const posterLayout = poster > 0 ? layoutFor(0, stageLeft, stageRight, room, unit, true) : null;
      ...
      if (wordState && wordState.main) {
        ...
        context.fillStyle = cssColor(mixColor(fieldColor, hexColor(colors.ink), 0.4));
        context.save();
      ...
      if (singer) {
        ...
        let lean;
        if (swingLate > 0.04) {
          const late = 0.75 + swingLate;
          lean = phase < late ? mixValue(-1, 1, snap(phase)) : mixValue(1, -1, snap(phase - late));
        } else lean = phase < 0.1 ? mixValue(-1, 1, snap(phase)) : 1 - 2 * easeInOutCubic((phase - 0.1) / 0.9); // one snap a beat
        const sway = clampRange(lean, -1, 1) * 0.1 * singerAwake * (1 - windUp) * (1 - poster);
        ...
        const lift = liftFor(260 * unit, singer.depth) - singer.depth * (bodyHeight + 40 * unit);
        ...
        context.fill();
        // the mouth: a notch cut from the lower front of the head, the jaw dropping with the
        // voice; and a small eye above it
        const jaw = (0.12 + 0.62 * level) * singerAwake;
        const corner = [headX + facing * headRadius * 0.28, headY + headRadius * 0.22];
        const upper = 0.2;
        context.fillStyle = field;
        context.beginPath();
        context.moveTo(corner[0], corner[1]);
        context.lineTo(headX + facing * headRadius * 1.3 * Math.cos(upper), headY + headRadius * 1.3 * Math.sin(upper));
        context.lineTo(headX + facing * headRadius * 1.3 * Math.cos(upper + jaw), headY + headRadius * 1.3 * Math.sin(upper + jaw));
        context.closePath();
        context.fill();
        context.beginPath();
        context.arc(headX + facing * headRadius * 0.42, headY - headRadius * 0.3, headRadius * 0.12, 0, Math.PI * 2);
        context.fill();
      }
      ...
      if (bassPlace) {
        ...
        if (hasDome) {
          ...
          const lift = hop * unit * size * bassAwake * (1 - poster) + liftFor(200 * unit, domeDepth) - domeDepth * (h + 40 * unit);
      ...
      if (drumPlace) {
        ...
        if (kit > 1) {
          const cymbal = (events, side, seed) => {
            ...
            disc(cx, cyC, radius * 0.62, radius * 0.2, colors.ink, seed, side * (0.08 + 0.45 * ring));
          };
      ...
      // Swing and push show as offsets from the cells' edges in a single frame.
      {
        const laneLeft = width * 0.03,
          ...
          laneBottom = height - (height - floor) * 0.08;
        ...
        const stampSize = rowHeight * 0.44;
