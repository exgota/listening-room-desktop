...
registerVisualizer({
  ...
  create() {
    ...
    function render(time, frame) {
      ...
      // A pale scene opens on its own field and takes a partner field (another of the pale
      // five, never a grey mix) for every second eight bars of it, so a long scene moves.
      if (pocketPale.includes(fieldName) && drop.phase !== 1 && !(peak && peak.kind === "chords")) {
        const opened = song.barIndex(song.scenes[sceneIndex].start + 0.01);
        if (Math.floor(Math.max(0, song.barIndex(time) - opened) / 8) % 2 === 1) fieldName = pocketPale[(pocketPale.indexOf(fieldName) + 3) % 5];
      }
      ...
      const posterLayout = poster > 0 ? layoutFor(0, stageLeft, stageRight, room, unit, true, width / 2) : null;
