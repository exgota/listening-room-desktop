...
# usage: sh review.sh <variant> <outdir> [scale] [pages] [songs...]
set -e
cd "$(dirname "$0")"
VARIANT=$1; OUT=$2; SCALE=${3:-0.5}; PAGES=${4:-1}
...
for SONG in $SONGS; do
  D=$(duration $SONG)
  node render.mjs --song $SONG --variant $VARIANT --out "$OUT/frames/$SONG" --from 0 --to $D --fps 2 --scale $SCALE --pages $PAGES
  python3 sheet.py --out "$OUT/sheets/$SONG" --cols 5 --rows 8 "$OUT"/frames/$SONG/*.png > /dev/null
  node render.mjs --song $SONG --variant $VARIANT --out "$OUT/stills" --times 0 --scale 0.5 --paused --prefix "$SONG-paused"
  for P in $(peaks $SONG); do
    FROM=$(python3 -c "print(round($P-2,1))"); TO=$(python3 -c "print(round($P+2,1))")
    node render.mjs --song $SONG --variant $VARIANT --out "$OUT/strip-frames/$SONG-$P" --from $FROM --to $TO --fps 10 --scale 0.25 --pages $PAGES --prefix s
    ...
    node render.mjs --song $SONG --variant $VARIANT --out "$OUT/stills" --times $P --scale 0.5 --prefix "$SONG-peak"
  done
done
